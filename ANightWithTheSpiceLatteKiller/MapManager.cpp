...
void MapManager::PrintMap(int colorOverrideIndex, bool excludePlayer) {
	...
	int blankSpaceY = (_printer->Csbi.dwMaximumWindowSize.Y - _map.size()) / 2;
	int blankSpaceX = (_printer->Csbi.dwMaximumWindowSize.X - _map[0].size()) / 2;

	CHAR_INFO blank;
	blank.Char.AsciiChar = ' ';
	blank.Attributes = _printer->MakeColor(BLACK, BLACK);
...
void MapManager::Init() 
{
	...
	int blankY = (_printer->Csbi.dwMaximumWindowSize.Y - mapSizeY) / 2;
	int blankX = (_printer->Csbi.dwMaximumWindowSize.X - mapSizeX) / 2;
